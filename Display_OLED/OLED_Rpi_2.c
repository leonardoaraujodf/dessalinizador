...
uint8_t SendData[] = {OLED_CONTROL_BYTE_DATA_STREAM};
...
void WriteCmdOLED(uint8_t *data,uint16_t size){
    if(write(i2c_fd,data,size) < size){
        printf("Error trying to write commands.\n");
    }
}

void WriteDataOLED(uint8_t *data,uint16_t size){
    int data_sent,i,j;
    //WriteCmdOLED(WriteInit,sizeof(WriteInit));
    //WriteCmdOLED(SendData,1);
    for(i=0;i<1024;i++){
        write(i2c_fd,&data[0],1);    
    }

    /*WriteCmdOLED(SendData,1);
    data_sent = write(i2c_fd, data, size);
    if(data_sent < size){
            printf("Error trying to write data commands.\n");
            printf("Data sent: %d\n",data_sent);
            close(i2c_fd);
            exit(-1);
        }
...
int main(void){
    ...
    WriteCmdOLED(SendData,1);
	while(1){		
		WriteDataOLED(test,sizeof(test));
